...
# Create Full Candidate Name
df["Candidate Name"] = df["First Name"].astype(str).str.strip() + " " + df["Last Name"].astype(str).str.strip()
...
if "filters_initialized" not in st.session_state:
    ...
    st.session_state.min_it = 0
    st.session_state.min_tr = 0
...
with st.sidebar:
    ...
    max_it = int(df["Total IT Experience"].max()) if not df["Total IT Experience"].dropna().empty else 10
    max_tr = int(df["Troubleshooting Experience"].max()) if not df["Troubleshooting Experience"].dropna().empty else 10
    ...
    st.session_state.game = st.radio("Gaming/Films Experience", options=["All","Yes","No"], index=["All","Yes","No"].index(st.session_state.game))
    st.session_state.min_it = st.slider("Min Total IT exp", min_value=0, max_value=max_it, step=1, value=int(st.session_state.min_it))
    st.session_state.min_tr = st.slider("Min Troubleshoot exp", min_value=0, max_value=max_tr, step=1, value=int(st.session_state.min_tr))
...
# Drop unwanted columns before showing
cols_to_drop = ["Feedback", "Comments", "GameFilmYN"]
df_display = df_f.drop(columns=[c for c in cols_to_drop if c in df_f.columns])

# Reorder columns so Candidate Name is first if exists
if "Candidate Name" in df_display.columns:
    cols = ["Candidate Name"] + [c for c in df_display.columns if c != "Candidate Name"]
